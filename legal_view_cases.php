...
$query = "SELECT DISTINCT c.*, e.full_name AS complainant,
...
// ردود الأطراف
$latestSession = $conn->query("
    SELECT session_id FROM sessions WHERE case_id=$case_id ORDER BY session_id DESC LIMIT 1
")->fetch_assoc();
$session_id_q = $latestSession['session_id'] ?? 0;
$responseSummary = '';
if($session_id_q > 0){
    $resps = $conn->query("
        SELECT e.full_name, sr.response
        FROM settlementresponses sr
        JOIN employees e ON sr.employee_id = e.employee_id
        WHERE sr.session_id = $session_id_q
    ");
    while($r = $resps->fetch_assoc()){
        $rb = ($r['response']=='ACCEPT') ? 'bg-success' : (($r['response']=='REJECT') ? 'bg-danger' : 'bg-warning');
        $rt = ($r['response']=='ACCEPT') ? 'قبل' : (($r['response']=='REJECT') ? 'رفض' : 'انتظار');
        $responseSummary .= "<div class='mb-1'><small class='badge $rb'>$rt</small> <small>{$r['full_name']}</small></div>";
    }
    if(empty($responseSummary)) $responseSummary = "<small class='text-muted'>لا توجد ردود</small>";
} else {
    $responseSummary = "<small class='text-muted'>لا توجد جلسة</small>";
}
?>
...
<a href="legal_dashboard.php" class="btn btn-outline-light">⬅ رجوع</a>
</div>
